...
Owned by the `business-analyst` agent (`.claude/agents/business-analyst.md`).
...
**Note on scope:** this doc doesn't yet have a row in `docs/README.md`'s
ownership index — that's a one-line addition to make once this file and
`AGENTS.md`/`docs/agent_governance.md` (currently on the still-open PR #197)
are both merged, so the edit doesn't collide with that PR.
