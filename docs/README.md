...
| What an AI coding agent may do unattended vs. needs sign-off for, and the agent roster | `docs/agent_governance.md` | Defines approval tiers (deploy targets, GRANT/DROP/VACUUM SQL, credentials, promotions), the 7-role functional roster, and maps it to the generated Claude and Codex agent surfaces. `AGENTS.md` is the general entry point for any coding agent; this is the specific authority policy. |
...
| Business case intake — what's been asked for, and how it reconciles against what's already built/decided | `docs/business_requirements.md` | Owned by the `architect` subagent. Every case gets a status (Proposed / Under review / Approved → issues filed / Deferred / Rejected) and a reconciliation section before anything becomes an issue. |
| Per-repo configuration the installed agent skills read — where issues live, the triage label vocabulary, and how skills consume domain docs | `docs/agents/` | Three files: `issue-tracker.md`, `triage-labels.md`, `domain.md`. Generated by `/setup-matt-pocock-skills` and safe to hand-edit; re-run that skill only to switch issue trackers or start over. Describes *how skills read this repo* — it does not grant permissions. `docs/agent_governance.md` still owns what any agent may do unattended, and `AGENTS.md` § "Skills vs. the roster" owns which pack skill defers to which subagent. |
...
| `docs/decisions/2026-08_xml_integrity_policy.md` | Whether malformed XML can partially land and where integrity is enforced. | **Proposed — awaiting Tier 2 sign-off** |
| `docs/decisions/2026-08_xml_namespace_identifiers.md` | How XML namespace-prefix spelling maps to Delta-safe identifiers and what source binding stability is required. | **Proposed — awaiting Tier 2 sign-off** |
